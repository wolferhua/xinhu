...
js.alert=function(msg,fun,tit,slx){
	if(!tit)tit='系统提示';
...
js.prompt=function(tit,msg,fun, txt,lxs){
	...
	if(!lxs)msg1 +='<input id="rockpromptinput" class="input" style="width:95%;border-radius:5px" value="'+txt+'" type="input">';
	if(lxs)msg1 +='<textarea id="rockpromptinput" class="input" style="width:95%;border-radius:5px;height:80px" type="input">'+txt+'</textarea>';
	msg1 += '</div>'
...
js.resizetan=function(act){
	...
	var h1 = o1.height();
	var w1 = o1.width();	
	var l=(winWb()-w1)*0.5,t=(winHb()-h1-50)*0.5;
